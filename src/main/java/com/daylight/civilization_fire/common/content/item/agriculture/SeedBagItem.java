    ...
    public InteractionResult useOn(UseOnContext useOnContext) {
        if (useOnContext.getPlayer() != null) {
            ...
            for (int i = 0; i < randomTimes; i++) {
                ResourceKey<Block> resourceKey = CivilizationFireUtil
                        .<ResourceKey<Block>>genericCast(PlantLoad.FRUIT_BLOCK_MAP.values()
                                .toArray()[random.nextInt(PlantLoad.FRUIT_BLOCK_MAP.values().size())]);
